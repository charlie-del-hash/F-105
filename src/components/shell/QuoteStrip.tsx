...
export function QuoteStrip() {
  ...
  return (
    <div className="flex min-w-0 items-center divide-x divide-line overflow-hidden font-data text-meta">
      {strip.map((s) => {
        const q = quotes[s];
        ...
        return (
          <Link key={s} href={`/markets/${s}`} className="tabular flex shrink-0 items-baseline gap-1.5 px-3 first:pl-0 hover:text-accent">
            ...
            {q && <Change value={q.change} pct={q.changePct} mode="pct" className="text-xs" />}
          </Link>
        );
      })}
    </div>
